...
Private Skills are the built-in tools that ship with every MoltGhost agent. They give OpenClaw the ability to take real actions in the world — sending tokens, browsing the web, executing code, calling APIs — while keeping all activity private by default.
